...
#fill nans on missing rows
avg_revenue = file["Revenue_(Millions)"].mean()
file["Revenue_(Millions)"].fillna(avg_revenue, inplace = True)

avg_metascore = file["Metascore"].mean()
file["Metascore"].fillna(avg_metascore, inplace = True)
...
average_revenue = file['Revenue_(Millions)'].mean()
...
average_revenue_2015_to_2017 = file.loc[(file['Year'] >= 2015) & (file['Year'] <= 2017), 'Revenue_(Millions)'].mean(skipna=True)
